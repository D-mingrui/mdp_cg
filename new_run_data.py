...
def read_data(data_path):
    ...
    with open(data_path) as origin_data_file:
        ...
        depot = DepotNode(data[0][0], data[0][1], data[0][2], data[0][4], data[0][5])
        requests = data[1:]
        ...
        for line in requests:
            if line[8] > 0:
                pickup_nodes.append(PickupNode(line[0], line[1], line[2], line[3], line[4], line[5], line[6], line[8]))
                # 寻找对应的送餐节点
                for line_inner in data:
                    if line_inner[0] == line[8]:
                        # 计算成本的最晚时间和右时间窗一致，所属餐馆和惩罚成本暂时都设为0
                        delivery_nodes.append(DeliveryNode(line_inner[0], line_inner[1], line_inner[2], line_inner[3],
    ...
    depot_path = r'C:\Users\32684\Desktop\mdp_data\LL_dataset\lc_101_depot_' + str(request_num) + '.txt'
    pickup_path = r'C:\Users\32684\Desktop\mdp_data\LL_dataset\lc_101_pickup_' + str(request_num) + '.txt'
    delivery_path = r'C:\Users\32684\Desktop\mdp_data\LL_dataset\lc_101_delivery_' + str(request_num) + '.txt'
